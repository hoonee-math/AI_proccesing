'''
>> Beta 16 프로그램 실행 창 가로 크기에 맞춰 동영상 실행
resize_frame() 함수 추가
process_video() 함수 수정
OnOpenDocument() 함수 수정
이후 use_camera() 가 작동하지 않는 문제 수정을 위해 use_camera 함수 수정
'''
...
# process_video 함수 수정
def process_video():
    global video_capture, video_playing, canvas, model, yolo_enabled, canvas_frame
