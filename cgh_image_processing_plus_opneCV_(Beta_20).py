...
def create_initial_screen():
    global main_frame, menu_frame, canvas_frame, initial_frame
    ...
    # 버튼 생성
    image_button = Button(bg_canvas, text="이미지 처리", command=switch_to_image_processing, **button_style)
    image_button.place(relx=0.35, rely=0.48, anchor="center")

    video_button = Button(bg_canvas, text="비디오 처리", command=switch_to_video_processing, **button_style)
    video_button.place(relx=0.35, rely=0.68, anchor="center")

    # 버튼에 마우스 오버 효과 추가
    image_button.bind("<Enter>", lambda e: e.widget.config(bg='#D4C2B5'))
    image_button.bind("<Leave>", lambda e: e.widget.config(bg='#C2906D'))
    video_button.bind("<Enter>", lambda e: e.widget.config(bg='#D4C2B5'))
    video_button.bind("<Leave>", lambda e: e.widget.config(bg='#C2906D'))
