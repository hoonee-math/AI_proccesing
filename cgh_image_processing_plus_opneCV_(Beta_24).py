...
>> Beta 24
가. 아직 해결되지 않은 문제점.
1. [파일 열기]를 통해 동영상 파일을 열어 재생 후 [카메라 사용]으로 넘어갈때 셧다운되는 문제.
2. [파일 열기]를 통해 동영상 파일을 열었을때 동영상이 바로 재생되지 않는 문제.
'''
...
def OnDraw():
    ...
    # 이미지 리사이즈
    resized_image = Image.new("RGB", (outW, outH))
    pixels = []
    for i in range(outH):
        for k in range(outW):
            r, g, b = outImage[RR][i][k], outImage[GG][i][k], outImage[BB][i][k]
            pixels.append((r, g, b))
    resized_image.putdata(pixels)
...
def process_video(): # Beta 24
    global video_capture, video_playing, canvas, model, yolo_enabled, canvas_frame, object_counts, tracked_ids
