...
>> Beta 26
1. 동영상 객체인식 정보 크게 표현
2. 사진, 동영상 효과 적용 내용 상태 표시줄 적용
add_status_text() 함수의 결과를 sbar에 표시하도록 select_submenu(), toggle_submenu(), update_status_bar(), process_video() 함수 수정
3. 누적 인식 객체수, 현재 화면 객체수 분리
update_object_count_display() 함수를 수정하여 오른쪽 캔버스에 정보 표시되도록 수정
process_video() 함수에 현재 화면 객체수를 파악하는 current_frame_counts 변수 추가
update_object_count_display(current_frame_counts) 함수에 변수를 받아와서 사용
'''
...
def OnDraw():
    ...
    # 이미지 크기 조정
    aspect_ratio = outH / outW
    if canvas_height / canvas_width < aspect_ratio:
    ...
    # OpenCV를 사용하여 이미지 리사이즈
    temp_image = np.zeros((outH, outW, 3), dtype=np.uint8)
    for i in range(outH):
        for k in range(outW):
            temp_image[i, k] = [outImage[BB][i][k], outImage[GG][i][k], outImage[RR][i][k]]

    resized_image = cv2.resize(temp_image, (new_width, new_height), interpolation=cv2.INTER_AREA)
    ...
    # 이미지 표시
    canvas.create_image((canvas_width - new_width) // 2, (canvas_height - new_height) // 2, image=photo, anchor=NW)
...
def process_video(): # Beta 24
    global video_capture, video_playing, canvas, model, yolo_enabled, canvas_frame, object_counts, tracked_ids, sbar    # beta 26 에서 추가 sbar
    ...
    if yolo_enabled and model is not None:
        try:
            ...
            frame_ = results[0].plot()  # 이 부분이 박스와 레이블을 그립니다.
...
def create_image_menu():
    ...
    # 상위 메뉴 버튼들
    pixel_btn = Button(menu_frame, text="화소 처리 기법", command=lambda: toggle_submenu(pixel_frame, pixel_btn))
    pixel_btn.pack(fill=X, padx=10, pady=5)

    geometric_btn = Button(menu_frame, text="기하학 처리", command=lambda: toggle_submenu(geometric_frame, geometric_btn))
    geometric_btn.pack(fill=X, padx=10, pady=5)

    area_btn = Button(menu_frame, text="화소 영역 처리", command=lambda: toggle_submenu(area_frame, area_btn))
    area_btn.pack(fill=X, padx=10, pady=5)

    opencv_btn = Button(menu_frame, text="Open CV 처리", command=lambda: toggle_submenu(opencv_frame, opencv_btn))
    ...
    Button(pixel_frame, text="동일 이미지", command=lambda: (var.set("동일"), select_submenu(equalImage, pixel_frame, "동일 이미지"))).pack(fill=X)
    Button(pixel_frame, text="밝게/어둡게", command=lambda: (var.set("밝게/어둡게"), select_submenu(addImage, pixel_frame, "밝게/어둡게"))).pack(fill=X)
    Button(pixel_frame, text="그레이스케일", command=lambda: (var.set("그레이"), select_submenu(grayImage, pixel_frame, "그레이스케일"))).pack(fill=X)
    Button(pixel_frame, text="반전", command=lambda: (var.set("반전"), select_submenu(reversedImage, pixel_frame, "반전"))).pack(fill=X)
    Button(pixel_frame, text='흑백', command=lambda: (var.set("흑백"), select_submenu(bwImage, pixel_frame, "흑백"))).pack(fill=X)
    Button(pixel_frame, text='포스터라이징', command=lambda: (var.set("포스터"), select_submenu(posterImage, pixel_frame, "포스터라이징"))).pack(fill=X)

    # 기하학 처리 서브메뉴
    Button(geometric_frame, text="확대/축소", command=lambda: select_submenu(zoomImage, geometric_frame, "확대/축소")).pack(fill=X)
    Button(geometric_frame, text="이동", command=lambda: select_submenu(moveImage, geometric_frame, "이동")).pack(fill=X)
    Button(geometric_frame, text='대칭', command=lambda: select_submenu(mirrorImage, geometric_frame, "대칭")).pack(fill=X)
    Button(geometric_frame, text='회전', command=lambda: select_submenu(rotateImage, geometric_frame, "회전")).pack(fill=X)

    # 화소 영역 처리 서브메뉴
    Button(area_frame, text="엠보싱(RGB)", command=lambda: select_submenu(embossImageRGB, area_frame, "엠보싱(RGB)")).pack(fill=X)
    Button(area_frame, text='엠보싱(HSV)', command=lambda: select_submenu(embossImageHSV, area_frame, "엠보싱(HSV)")).pack(fill=X)
    Button(area_frame, text='블러링(RGB)', command=lambda: select_submenu(blurImageRGB, area_frame, "블러링(RGB)")).pack(fill=X)
    Button(area_frame, text='블러링(HSV)', command=lambda: select_submenu(blurImageHSV, area_frame, "블러링(HSV)")).pack(fill=X)
    Button(area_frame, text='샤프닝(RGB)', command=lambda: select_submenu(sharpRGB, area_frame, "샤프닝(RGB)")).pack(fill=X)
    Button(area_frame, text='샤프닝(HSV)', command=lambda: select_submenu(sharpHSV, area_frame, "샤프닝(HSV)")).pack(fill=X)
    Button(area_frame, text='경계선(RGB)', command=lambda: select_submenu(lineRGB, area_frame, "경계선(RGB)")).pack(fill=X)
    Button(area_frame, text='경계선(HSV)', command=lambda: select_submenu(lineHSV, area_frame, "경계선(HSV)")).pack(fill=X)
    Button(area_frame, text='가우시안(RGB)', command=lambda: select_submenu(gaussianRGB, area_frame, "가우시안(RGB)")).pack(fill=X)
    Button(area_frame, text='가우시안(HSV)', command=lambda: select_submenu(gaussianHSV, area_frame, "가우시안(HSV)")).pack(fill=X)

    # Open CV 처리 서브메뉴
    Button(opencv_frame, text='동일 이미지', command=lambda: select_submenu(equalImageCV, opencv_frame, "동일 이미지")).pack(fill=X)
    Button(opencv_frame, text='그레이 이미지', command=lambda: select_submenu(grayscaleImageCV, opencv_frame, "그레이 이미지")).pack(fill=X)
    Button(opencv_frame, text='HSV 변환', command=lambda: select_submenu(hsvImageCV, opencv_frame, "HSV 변환")).pack(fill=X)
    Button(opencv_frame, text='binary 이미지', command=lambda: select_submenu(binaryImageCV, opencv_frame, "binary 이미지")).pack(fill=X)
    Button(opencv_frame, text='적응형 이미지', command=lambda: select_submenu(binary2ImageCV, opencv_frame, "적응형 이미지")).pack(fill=X)
    Button(opencv_frame, text='엠보싱 이미지', command=lambda: select_submenu(embossImageCV, opencv_frame, "엠보싱 이미지")).pack(fill=X)
    Button(opencv_frame, text='카툰 이미지', command=lambda: select_submenu(cartonImageCV, opencv_frame, "카툰 이미지")).pack(fill=X)
    Button(opencv_frame, text='얼굴인식', command=lambda: select_submenu(faceDetectCV, opencv_frame, "얼굴인식")).pack(fill=X)
    Button(opencv_frame, text='코인식', command=lambda: select_submenu(noseDetectCV, opencv_frame, "코인식")).pack(fill=X)

def toggle_submenu(frame, button):
    global selected_main_menu
    if selected_main_menu:
        selected_main_menu.config(font=('TkDefaultFont', 10, 'normal'))
...
def select_submenu(command, frame, name):
    ...
    for btn in frame.winfo_children():
        btn.config(font=('TkDefaultFont', 10, 'normal'))
    for btn in frame.winfo_children():
...
def create_video_menu():
    global menu_frame, video_path, person_count_label, yolo_button, object_count_label
    for widget in menu_frame.winfo_children():
        widget.destroy()
    ...
    # YOLO 및 카메라 섹션
    yolo_button = Button(menu_frame, text="YOLO 적용" if not yolo_enabled else "YOLO 비활성화", command=toggle_yolo)
