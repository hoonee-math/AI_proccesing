...
>> Beta 30
코드 정리
중복되거나, 사용하지 않은 import 정리
mask rgb, hsv 미적용 함수에 적용 & hsv mask 함수 개선
'''
...
def create_image_menu():
    ...
    Button(area_frame, text='경계선(HSV)', command=lambda: select_submenu(lineHSV, area_frame, "경계선(HSV)"), font=default_font).pack(fill=X)    # Beta 30 mask 함수 개선 후 다시 추가
    ...
    Button(area_frame, text='가우시안(HSV)', command=lambda: select_submenu(gaussianHSV, area_frame, "가우시안(HSV)"), font=default_font).pack(fill=X)    # Beta 30 mask 함수 개선 후 다시 추가
...
def create_video_menu():
    ...
    global object_threshold, count_threshold # Beta 29 객체 수 파악용 전역변수
    for widget in menu_frame.winfo_children():
        widget.destroy()
...
def sharpHSV():
    ...
    #########################
    OnDraw()
def maskRGB(a,b1,c,d,e,f,g1,h,i,j):
    # Beta 30 j 변수 추가, 마크스값계산(회선연산) 후 픽셀 값 후처리 +j
    global window, canvas, paper, inImage, outImage
    global inH, inW, outH, outW, inPhoto, outPhoto, filename
    outH = inH  # 출력 이미지의 크기를 결정
    outW = inW
    ...
    mask = [    [  a, b1, c],
                [  d, e, f],
                [  g1, h, i]  ]
    if j == None:   # Beta 30 mask 함수 개선 후 추가
        j = 0
    # 임시 출력, 출력 영상 확보
    tmpInImage = malloc3D(inH+2, inW+2, RGB)
    tmpOutImage = malloc3D(inH, inW, RGB)
    ...
    for rgb in range(RGB):
        for i in range(outH):
            for k in range(outW):
                if (tmpOutImage[rgb][i][k] + j < 0):
                    tmpOutImage[rgb][i][k] = 0
                elif (tmpOutImage[rgb][i][k] + j > 255):
                    tmpOutImage[rgb][i][k] = 255
                else:
                    tmpOutImage[rgb][i][k] += j
    ...
    #########################
    OnDraw()
def maskHSV(a,b1,c,d,e,f,g1,h1,i,j):
    # Beta 30 j 변수 추가, 마크스값계산 후 픽셀 값 후처리 +j
    global window, canvas, paper, inImage, outImage
    global inH, inW, outH, outW, inPhoto, outPhoto, filename
    # (6-3) 입력 RGB 메모리 -> 입력 HSV 메모리
    inImageHSV = malloc3D(inH, inW, RGB)
    ...
    for i in range(outH):
       for k in range(outW):
           if(tmpOutImageV[i][k] + j < 0) :
               tmpOutImageV[i][k] = 0
           elif (tmpOutImageV[i][k] + j > 255) :
               tmpOutImageV[i][k] = 255
           else :
               tmpOutImageV[i][k] += j
    # 임시 출력을 RGB로 변환해서 실제로 출력하기
