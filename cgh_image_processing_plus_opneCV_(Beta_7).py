'''
Beta 7 _ 실패 버전
kkh tp (beta 2) import 해서 기능 구현하기
cgh_class_Yolo_by_kkh_version 로 이름 바꿔서 진행
'''
...
# Beta 7: YoloProcessor 클래스 import
import cgh_class_Yolo_by_kkh_version as yolo
...
def OnOpenDocument():   # Beta 4 수정사항, steup_ui() 함수 추가
    global filename, inPhoto, inImage, outImage, inH, inW, outH, outW, video_capture, canvas, video_path, yolo_processor # Beta 7 수정 video_path, yolo_processor 변수 추가
    ...
    if file_extension in ['jpg', 'png', 'bmp', 'tif']:
        ...
        # Beta 7 비디오 파일 처리 수정
        video_path = filename
        yolo_processor = yolo.YoloProcessor()
        create_video_menu()
        '''
        # Beta 5: 비디오 파일 처리 추가
    ...
    # # 비디오용 캔버스 생성 # Beta 7 삭제
    # if canvas:
    #     canvas.destroy()
    # canvas = Canvas(canvas_frame, width=video_capture.get(cv2.CAP_PROP_FRAME_WIDTH),
...
def create_video_menu():
    ...
    # YOLO 효과 버튼 추가
    Button(menu_frame, text="모자이크", command=lambda: yolo_processor.toggle_mosaic()).pack(fill=X, padx=10, pady=5)
    Button(menu_frame, text="블러", command=lambda: yolo_processor.toggle_blur()).pack(fill=X, padx=10, pady=5)
    Button(menu_frame, text="선명화", command=lambda: yolo_processor.toggle_sharpen()).pack(fill=X, padx=10, pady=5)
    Button(menu_frame, text="그레이스케일", command=lambda: yolo_processor.toggle_grayscale()).pack(fill=X, padx=10, pady=5)
    Button(menu_frame, text="반전", command=lambda: yolo_processor.toggle_invert()).pack(fill=X, padx=10, pady=5)
    Button(menu_frame, text="미러", command=lambda: yolo_processor.toggle_mirror()).pack(fill=X, padx=10, pady=5)
...
# Beta 5: 비디오 재생 함수
def play_video():
    global video_capture, video_playing, canvas, video_path, yolo_processor # Beta 7 수정 video_path, yolo_processor 추가
    if video_capture is None:
        return

    video_playing = cv2.VideoCapture(video_path)
    if not video_capture.isOpened():
        messagebox.showerror("에러", "비디오 파일을 열 수 없습니다.")
        return
...
def video_play_thread():
    global video_capture, video_playing, canvas, yolo_processor
    while video_playing:
        ...
        frame = yolo_processor.process_frame(frame)
...
def use_camera():
    global video_capture, video_playing, canvas, video_path, yolo_processor
    video_path = 0  # 카메라 사용
    yolo_processor = yolo.YoloProcessor()
    play_video()
    ...
    while video_playing:
        ret, frame = video_capture.read()
        if ret:
            frame_ = yolo_processor.process_frame(frame)

            # 상태 텍스트 추가
            status_text = yolo_processor.get_status_text()
            y0, dy = 30, 40
...
video_path = None       # Beta 5: 비디오 관련 전역 변수 추가 # Beta 7: 전역 변수 추가
yolo_processor = None   # Beta 7: 전역 변수 추가
